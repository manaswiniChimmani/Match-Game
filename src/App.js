...
import ThumbnailImagesList from './components/ThumbnailImagesList'
...
class App extends Component {
  state = {
    thumbnailList: tabsList[0].tabId,
  ...
  tick = () => {
    this.setState(prevState =>
      prevState.time > 0
        ...
        : {time: 0, isGameOver: true},
    )
  }

  display = id => {
    this.setState({thumbnailList: id})
  }
  ...
  render() {
    const {thumbnailList, randomImg, score, isGameOver, time} = this.state
    // console.log('render')
    const ImgUrl = randomImg.imageUrl

    const listOfThumbNail = imagesList.filter(
      each => each.category === thumbnailList,
    )
            ...
            <ul className="table-tab-cont">
              {tabsList.map(each => (
                <TabsList
                  ...
                  activeTab={thumbnailList === each.tabId}
                />
              ))}
            </ul>
